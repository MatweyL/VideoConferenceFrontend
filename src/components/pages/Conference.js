    ...
    useEffect(() => {
        getConference(document.location.pathname).then(r => {
            if (r.error) {
                ...
                let displayName = u.user_verbose.user.username;
                const redirectURL = `${process.env.REACT_APP_CONFERENCE_URL}/join?room_id=${u.conference_id}&display_name=${displayName}&is_creator=${is_creator}&mute_audio=0&mute_video=0&video_token=${getToken()}`;
                // alert(redirectURL)
